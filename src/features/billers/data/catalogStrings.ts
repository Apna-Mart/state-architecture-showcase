...
export const providerPrefixes: Record<string, Localized> = {
...
export const paramLabels: Record<string, Localized> = {
...
export const paramHints: Record<string, Localized> = {
...
export const enterHint = (label: string, language: SupportedLanguage): string => {
  if (language === 'hi') return `${label} दर्ज करें`;
  if (language === 'ar') return `أدخل ${label}`;
  return `Enter ${label}`;
};

export const billerName = (prefixId: string, categoryId: string, language: SupportedLanguage): string => {
